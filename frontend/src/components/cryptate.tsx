...
export default function Cryptate() {
  ...
  const [opened, { open, close }] = useDisclosure(false, {onClose: () => {
    setFail({failed: false, success: false, message: '', link: <a></a>});
  }});
  ...
  const form = useForm({
    ...
    validate: {
      key: (value) => (value < 0 ? 'Key must be positive' : null),
      message: (value) => (value === '' ? 'Message cannot be empty' : null),
    },
  });
  ...
  return (
    <>
      ...
      <CryptateModal
        status={failed}
        disclousure={{ opened: opened, open: open, close: close }}
      ></CryptateModal>
    </>
  );
}
